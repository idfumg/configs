...
--       link = tostring (40)      -- for testing
       local link = awful.util.pread("sudo iwconfig wlp7s0 | awk -F '=' '/Quality/ {print $2}' | cut -d '/' -f 1")
...
-- Register widget
vicious.register(wifidown, vicious.widgets.net, "${wlp7s0 down_kb}")
...
-- Register widget
vicious.register(wifiup, vicious.widgets.net, "${wlp7s0 up_kb}")
